...
var carousel = new _module2.default('carousel');

carousel.createCarousel = function (container, settings) {
  if (!(0, _jquery2.default)(container).length) {
    return;
  }

  return (0, _jquery2.default)(container).slick(_jquery2.default.extend(true, {}, CAROUSEL_SETTINGS, settings));
};
...
var intro = new _module2.default('intro'),
    body = intro.alias.body;

intro.createSlider = function (container, settings) {
  if (!(0, _jquery2.default)(container).length) {
    return;
  }

  return (0, _jquery2.default)(container).slick(_jquery2.default.extend(true, {}, INTRO_SETTIGNS, settings));
};

intro.finish = function (e, slick, currentSlide) {
  if (currentSlide === SLIDES_NUM - 1) {
    intro.state.introContainer.slick('unslick');
    intro.state.introWrapper.animate({
      top: '-100%'
    }, {
      duration: 1000,
      done: function done() {
        body.removeClass('eui-body--hidden');
        intro.state.introWrapper.remove();
        body.off('mousewheel DOMMouseScroll', intro.onWheel);
      }
    });
  }
};

intro.onWheel = function (e) {
  if (e.originalEvent.wheelDelta > 0 || e.originalEvent.detail < 0) {
    intro.state.introContainer.slick('slickPrev');
  } else {
    intro.state.introContainer.slick('slickNext');
  }
};
...
var slider = new _module2.default('slider');

slider.createSlider = function (container, settings) {
  if (!(0, _jquery2.default)(container).length) {
    return;
  }

  return (0, _jquery2.default)(container).slick(_jquery2.default.extend(true, {}, SLIDER_SETTINGS, settings));
};
...
var body = _carousel2.default.alias.body,
    articles = (0, _jquery2.default)('#articles'),
    introContainer = (0, _jquery2.default)('#intro');

_intro2.default.extend({
  introContainer: introContainer,
  introWrapper: (0, _jquery2.default)('.intro')
});
_intro2.default.createSlider('#intro', {
  nextArrow: '.intro__arrow',
  prevArrow: ''
});
introContainer.on('afterChange', _intro2.default.finish);
body.on('mousewheel DOMMouseScroll', _intro2.default.onWheel);

_slider2.default.createSlider('#banner', {
  arrows: false
});

articles.on('init', function (e, slick) {
  slick.$slides.eq(slick.currentSlide).addClass('article--active');
});
_carousel2.default.createCarousel('#articles', {
  centerMode: true,
  variableWidth: true,
  prevArrow: '[data-relation="articles-prev"]',
  nextArrow: '[data-relation="articles-next"]',
  responsive: [{
    breakpoint: 1023,
    settings: {
      centerMode: false,
